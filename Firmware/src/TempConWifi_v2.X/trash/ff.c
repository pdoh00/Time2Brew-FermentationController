...
FRESULT f_write_stream(
    ...
    DWORD clst, sect;
    UINT wcnt, cc;
    unsigned long wbuff_Address = StreamOffset;
    BYTE csect;
    ...
    *bw = 0; /* Clear write byte counter */
    ...
    for (; btw; /* Repeat until all data written */
            wbuff_Address += wcnt, fp->fptr += wcnt, *bw += wcnt, btw -= wcnt) {
        if ((fp->fptr % SS(fp->fs)) == 0) { /* On the sector boundary? */
            csect = (BYTE) (fp->fptr / SS(fp->fs) & (fp->fs->csize - 1)); /* Sector offset in the cluster */
            if (!csect) { /* On the cluster boundary? */
                if (fp->fptr == 0) { /* On the top of the file? */
            ...
            if (cc) { /* Write maximum contiguous sectors directly */
                if (csect + cc > fp->fs->csize) /* Clip at cluster boundary */
                    cc = fp->fs->csize - csect;
                if (disk_write_stream(fp->fs->drv, wbuff_Address, sect, cc, StreamCpy) != RES_OK)
                    ABORT(fp->fs, FR_DISK_ERR);
#if _FS_MINIMIZE <= 2
#if _FS_TINY
                if (fp->fs->winsect - sect < cc) { /* Refill sector cache if it gets invalidated by the direct write */
                    StreamCpy(fp->fs->win, wbuff_Address + ((fp->fs->winsect - sect) * SS(fp->fs)), SS(fp->fs));
                    //mem_cpy(fp->fs->win, wbuff_Address + ((fp->fs->winsect - sect) * SS(fp->fs)), SS(fp->fs));
                    fp->fs->wflag = 0;
                }
#else
                if (fp->dsect - sect < cc) { /* Refill sector cache if it gets invalidated by the direct write */
                    StreamCpy(fp->buf, wbuff_Address + ((fp->dsect - sect) * SS(fp->fs)), SS(fp->fs));
                    //mem_cpy(fp->buf, wbuff_Address + ((fp->dsect - sect) * SS(fp->fs)), SS(fp->fs));
                    fp->flag &= ~FA__DIRTY;
                }
#endif
#endif
        ...
        if (move_window(fp->fs, fp->dsect) != FR_OK) /* Move sector window */
            ABORT(fp->fs, FR_DISK_ERR);
        mem_cpy(&fp->fs->win[fp->fptr % SS(fp->fs)], wbuff_Address, wcnt); /* Fit partial sector */
        fp->fs->wflag = 1;
#else
        StreamCpy(&fp->buf[fp->fptr % SS(fp->fs)], wbuff_Address, wcnt); /* Fit partial sector */
        //mem_cpy(&fp->buf[fp->fptr % SS(fp->fs)], wbuff_Address, wcnt); /* Fit partial sector */
        fp->flag |= FA__DIRTY;
#endif
    }
