...
#define CS_LOW()  SET_SDCARD_CS(0)	/* MMC CS = L */
#define CS_HIGH() SET_SDCARD_CS(1)	/* MMC CS = H */
...
#define FCLK_SLOW() SetSPIBaudRate(200000)  /* Set slow clock (100k-400k) */
#define	FCLK_FAST()	SetSPIBaudRate(SD_FAST_BAUD)	/* Set fast clock (depends on the CSD) */

#define SD_SPIBUFFER    SPI1BUF
#define SD_RXFULL   SPI1STATbits.SPIRBF
...
static volatile
UINT FatFS_Timer1, FatFS_Timer2; /* 1000Hz decrement timer */
...
int wait_ready(void) {
    ...
    FatFS_Timer2 = 500; /* Wait for ready in timeout of 500ms */
    do {
        d = xchg_spi(0xFF);
    } while ((d != 0xFF) && FatFS_Timer2);
...
DSTATUS disk_initialize(
    ...
    if (send_cmd(CMD0, 0) == 1) { /* Enter Idle state */
        FatFS_Timer1 = 1000; /* Initialization timeout of 1000 msec */
        if (send_cmd(CMD8, 0x1AA) == 1) { /* SDv2? */
            for (n = 0; n < 4; n++) ocr[n] = xchg_spi(0xFF); /* Get trailing return value of R7 resp */
            if (ocr[2] == 0x01 && ocr[3] == 0xAA) { /* The card can work at vdd range of 2.7-3.6V */
                while (FatFS_Timer1 && send_cmd(ACMD41, 0x40000000)); /* Wait for leaving idle state (ACMD41 with HCS bit) */
                if (FatFS_Timer1 && send_cmd(CMD58, 0) == 0) { /* Check CCS bit in the OCR */
            ...
            while (FatFS_Timer1 && send_cmd(cmd, 0)); /* Wait for leaving idle state */
            if (!FatFS_Timer1 || send_cmd(CMD16, 512) != 0) /* Set read/write block length to 512 */
                ty = 0;
        }
    }
    CardType = ty;
...
DRESULT disk_write_stream(
        BYTE pdrv, /* Physical drive nmuber (0) */
        unsigned long StreamAddress, /* Pointer to the data to be written */
        DWORD sector, /* Start sector number (LBA) */
        UINT count, /* Sector count (1..128) */
        void (*StreamCpy)(BYTE *buff, unsigned long address, UINT bCount)
        ) {
    ...
    if (count == 1) { /* Single block write */
        StreamCpy(buff, StreamAddress, 512);
